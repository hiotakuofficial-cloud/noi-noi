...
function playEpisode($id, $episode_id) {
    ...
    // Extract iframe URLs from data attributes
    preg_match_all('/data-(?:hindi|multi|sub)-path="([^"]+)"/', $response, $iframe_matches);
    
    if (!empty($iframe_matches[1])) {
        $video_urls = array_unique($iframe_matches[1]);
        
        // Decode HTML entities
        $video_urls = array_map('html_entity_decode', $video_urls);
        
        // Prioritize Hindi dubbed if available
        $hindi_url = null;
        foreach ($video_urls as $video_url) {
            if (strpos($video_url, 'strmup.to') !== false) {
                $hindi_url = $video_url;
                break;
            }
        }
        
        // Use Hindi URL as primary if found
        if ($hindi_url) {
            array_unshift($video_urls, $hindi_url);
            $video_urls = array_unique($video_urls);
        }
        
        // Re-index array to fix 0,1,2,3... sequence
